...
This library provides a Markdown parser that converts a DMN table in Markdown format into the Haskell Decision AST. Sample translations to Python and JavaScript are also provided for reference.
...
```bash
cabal run dmn-exe inputfile.md
```
...
5. Function calls are in a prolog-style format, where outputs are treated as variables and can be reused if there are multiple calls
   * There should be a space between table declarations and function calls
    ...
    function2 (outputA, outputB) // outputA from function1 is used as an input for function2
    ```
...
* **Multi-hit policies** (such as collect and rule order) return lists in python.
