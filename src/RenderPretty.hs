...
instance Render Expr where
  ...
  renderAtPrio p (Let d e)          = parensIf (p > 0) ("let\n" <> indentText 2 (renderAtPrio 0 d) <> "in " <> render e)
...
indentText n txt = Text.unlines $ map (Text.replicate n " " <>) (Text.lines txt)
...
renderBuiltin p And        exprs        = parensIf (p > 3) (Data.Text.intercalate "\n&& " (map (renderAtPrio 3) exprs))
